...
describe("rankSearchResults", () => {
	...
	it("keeps the best semantic match ahead of a low-relevance recent note (food-query regression)", () => {
		...
		const semanticResults: Parameters<typeof rankSearchResults>[0]["semanticResults"] = [
			{
				path: "Cooking Mediterranean Recipes.md",
				name: "Cooking Mediterranean Recipes",
				score: 0.324,
				matchBadges: ["semantic"],
			},
		];
		for (let i = 2; i <= 16; i++) {
			semanticResults.push({
				path: `filler-sem-${i}.md`,
				name: `filler ${i}`,
				score: 0.2 - i * 0.005,
				matchBadges: ["semantic"],
			});
		}
		semanticResults.splice(16, 0, {
			path: "Large Notes/Storage Engines and Indexing.md",
			name: "Storage Engines and Indexing",
			score: 0.091,
			matchBadges: ["semantic"],
		});
		semanticResults.splice(18, 0, {
			path: "Large Notes/Distributed Systems Deep Dive.md",
		...
		for (let i = 1; i <= 7; i++)
			lexicalResults.push({ path: `filler-lex-${i}.md`, name: `flex ${i}`, score: 20 - i });
		lexicalResults.push({
			path: "Large Notes/Storage Engines and Indexing.md",
			name: "Storage Engines and Indexing",
			score: 1.299,
	...
	it("gives a stronger semantic match a higher base score at an adjacent RRF rank", () => {
		...
		expect((results[0]?.rankingDebug?.baseScore ?? 0) - (results[1]?.rankingDebug?.baseScore ?? 0)).toBeGreaterThan(
			0,
		);
	});
	...
	it("does not let recency-gated notes dilute an eligible note's tiebreaker", () => {
		...
		const scoreOf = (results: ReturnType<typeof run>) => results.find((r) => r.path === "near.md")?.score ?? 0;
