...
export function firstKey<T extends Record<string, unknown>>(
	value: T | undefined,
): string | undefined {
	if (!value) {
		return undefined;
...
export function normalizeDescriptor(
	value: string | BuiltInProviderModelMapEntry,
): BuiltInProviderModelMapEntry {
...
export function createChatFactories(
	...
	return Object.entries(entries).reduce<Record<string, ChatModelFactory>>(
		(acc, [alias, descriptor]) => {
...
export function createEmbeddingFactories(
	...
	return Object.entries(entries).reduce<Record<string, EmbeddingModelFactory>>(
		(acc, [alias, descriptor]) => {
			const normalized = normalizeDescriptor(descriptor);
			acc[alias] = factoryCreator(normalized);
			return acc;
