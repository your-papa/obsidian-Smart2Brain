...
export class PluginDataStore {
	...
	getSelEmbedModel<P extends EmbedProviders>(provider: P): GetProviderConfig<P>["selEmbedModel"] {
		return this.#data.providerConfig[provider].selEmbedModel;
	}

	selectEmbedModel<P extends EmbedProviders>(provider: P, value: string) {
		if (!this.#data.providerConfig[provider].embedModels.has(value)) throw new SetEmbedModelError(provider, value);
	...
	// --- Gen Model Management (Map-based) ---

	getSelGenModel<P extends GenProviders>(provider: P): GetProviderConfig<P>["selGenModel"] {
		return this.#data.providerConfig[provider].selGenModel;
	}

	selectGenModel<P extends GenProviders>(provider: P, value: string) {
		if (!this.#data.providerConfig[provider].genModels.has(value)) throw new SetGenModelError(provider, value);
		this.#data.providerConfig[provider].selGenModel = value;
		this.saveSettings();
	}
	...
	getProviderSelGenModel<K extends keyof PluginData["providerConfig"]>(provider: K): string | undefined {
		return (this.#data.providerConfig[provider] as any).selGenModel;
	}
	setProviderSelGenModel<K extends keyof PluginData["providerConfig"]>(provider: K, value: string) {
