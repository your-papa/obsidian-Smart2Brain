...
export class DexiePersistence implements IChatPersistence {
	...
	private async internalIncrementMsgCount(chatId: UUIDv7): Promise<void> {
		...
		const count = await this.db.messagePointers.where("chatId").equals(chatId).count();
		await this.db.chats.update(chatId, { msgCount: count });
	}

	private async pointerExists(chatId: UUIDv7, messageId: UUIDv7): Promise<boolean> {
		return (await this.db.messagePointers.where("[chatId+messageId]").equals([chatId, messageId]).count()) > 0;
	}
	...
	async listChats(): Promise<ChatPreview[]> {
		const rows = await this.db.chats.orderBy("lastAccessed").reverse().toArray();
	...
	async branchOffFromMessage(
		...
		return this.db.transaction("rw", this.db.chats, this.db.messagePointers, this.db.messagePairs, async () => {
			...
			const cutoffPointer = await this.db.messagePointers
				...
				.first();
			if (!cutoffPointer) {
				throw new Error(`cutoffMessageId ${cutoffMessageId} not found in parent chat ${parentChatId}`);
			}
			...
			const parentPointers = await this.db.messagePointers
				.where("[chatId+messageId]")
				.between([parentChatId, Dexie.minKey], [parentChatId, cutoffMessageId])
				.toArray();
	...
	async updateChatMeta(id: UUIDv7, patch: Partial<Omit<ChatRecordMeta, "UUIDv7" | "msgCount">>): Promise<void> {
		const update: Partial<ChatRow> = {};
		if (patch.title !== undefined) update.title = patch.title;
		if (patch.lastAccessed !== undefined) update.lastAccessed = patch.lastAccessed.toISOString();
		...
		try {
			const deleted = await this.db.transaction(
				...
				async () => {
					...
					const pointers = await this.db.messagePointers.where("chatId").equals(chatId).toArray();
					...
					if (msgIds.length) {
						const stillReferenced = new Set(
							await this.db.messagePointers.where("messageId").anyOf(msgIds).distinct().keys(),
						);
	...
	async getMessages(chatId: UUIDv7, options: ListMessagesOptions = {}): Promise<MessagePair[]> {
	...
	async getMessage(chatId: UUIDv7, messageId: UUIDv7): Promise<MessagePair | null> {
		const pointer = await this.db.messagePointers.where("[chatId+messageId]").equals([chatId, messageId]).first();
	...
	async addMessage(chatId: UUIDv7, message: MessagePair): Promise<void> {
		await this.db.transaction("rw", this.db.messagePairs, this.db.messagePointers, this.db.chats, async () => {
			const existingPair = await this.db.messagePairs.get(message.id);
			if (existingPair) {
				throw new Error(`Message with id ${message.id} already exists (add to chat ${chatId}).`);
			}
	...
	async dropHistoryAt(chatId: UUIDv7, messageId: UUIDv7): Promise<boolean> {
		return this.db.transaction("rw", this.db.messagePointers, this.db.messagePairs, this.db.chats, async () => {
			...
			const stillReferenced = new Set(
				await this.db.messagePointers.where("messageId").anyOf(ids).distinct().keys(),
			);
	...
	async updateAssistantMessagePartial(
		...
		await this.db.transaction("rw", this.db.messagePointers, this.db.messagePairs, async () => {
	...
	async deleteMessage(chatId: UUIDv7, messageId: UUIDv7): Promise<boolean> {
		return this.db.transaction("rw", this.db.messagePointers, this.db.messagePairs, this.db.chats, async () => {
			...
			// Remove pointer
			await this.db.messagePointers.where("[chatId+messageId]").equals([chatId, messageId]).delete();

			// If no more pointers reference this message, delete the message row
			const stillRefCount = await this.db.messagePointers.where("messageId").equals(messageId).count();
			if (stillRefCount === 0) {
