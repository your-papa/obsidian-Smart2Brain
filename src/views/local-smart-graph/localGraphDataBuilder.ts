...
export interface LocalSemanticGraphOptions {
	threshold?: number;
}
...
export function buildLocalWikiGraph(app: App, activePath: string): GraphData {
	const fileMap = getMarkdownFileMap(app);
	if (!fileMap.has(activePath)) {
		return { nodes: [], edges: [] };
	}

	const localPaths = new Set<string>([activePath]);
	...
	const edges = buildEdgesForPaths(app, localPaths);
	const nodes = createNodes(fileMap, localPaths, edges, activePath);
...
export function buildLocalSemanticGraph(
	...
	options: LocalSemanticGraphOptions = {},
): GraphData {
	const fileMap = getMarkdownFileMap(app);
	if (!fileMap.has(activePath)) {
		return { nodes: [], edges: [] };
	...
	const threshold = options.threshold ?? DEFAULT_LOCAL_SEMANTIC_THRESHOLD;
...
export function mergeLocalGraph(wikiGraph: GraphData, semanticGraph: GraphData, activePath: string): GraphData {
