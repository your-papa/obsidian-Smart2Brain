<script lang="ts">
  ...
  let chatModels = $derived.by<Record<string, ChatModelConfig>>(() => {
    ...
    const allowedKeys = new Set(
      (models ?? [])
        .map((m: string | { key?: string }) => (typeof m === "string" ? m : m?.key))
        .filter(Boolean),
    );
    ...
    return Object.fromEntries(
      Object.entries(configuredChatModels).filter(([key]) => allowedKeys.has(key)),
    );
  });
  ...
  let selectedModel = $derived(
    !isPending && !isError && models.length > 0 ? models[0] : configuredModels[0],
  );

  let unconfiguredModels: string[] = $derived(
    models.filter((model: string) => !configuredModels.includes(model)),
  );
