...
export default class SmartSecondBrain {
  ...
  constructor(plugin: SecondBrainPlugin, pluginDir: string) {
    this.plugin = plugin;
  ...
  async init() {
    ...
    if (get(papaState) !== "indexing-pause") {
      ...
      try {
        await this.papa.init({
          ...
          selEmbedModel,
          selGenModel,
          langsmithApiKey: debuggingLangchainKey || undefined,
      ...
      if (await this.app.vault.adapter.exists(this.getVectorStoreFile())) {
        const vectorStoreData = await this.app.vault.adapter.readBinary(
          this.getVectorStoreFile(),
        );
      ...
      await this.createPapaObsidianFiles(
        mdFiles.filter((mdFile: TFile) => {
          for (const exclude of excludeFF)
            if (wildTest(exclude, mdFile.path)) return false;
          return true;
        }),
      ),
    );
    ...
    try {
      ...
      for await (const result of this.papa.embedDocuments(docs)) {
        needsSave =
          (!this.needsToSaveVectorStoreData && result.numAdded > 0) ||
          result.numDeleted > 0;
        const progress =
          ((result.numAdded + result.numSkipped) / docs.length) * 100;
        papaIndexingProgress.set(Math.max(progress, get(papaIndexingProgress)));
        const currentTime = Date.now();
        timePerDoc.push(currentTime - lastTime);
        const numberOfDocumentsRemaining =
          docs.length * (1 - get(papaIndexingProgress) / 100);
        const averageTimePerDocumentInSeconds =
          timePerDoc.reduce((a, b) => a + b, 0) /
          timePerDoc.length /
          (result.numAdded + result.numSkipped - lastTotalDocs) /
          1000;
        lastTotalDocs = result.numAdded + result.numSkipped;
        if (timePerDoc.length > 5) {
          timePerDoc.shift();
          papaIndexingTimeLeft.set(
            Math.ceil(
              averageTimePerDocumentInSeconds * numberOfDocumentsRemaining,
            ),
          );
        }
  ...
  canRunPapa() {
    ...
    if (get(papaState) === "running")
      return new Notice(t("notice.still_running"), 4000) && false;
    else if (
      get(papaState) === "indexing" ||
      get(papaState) === "indexing-pause" ||
      get(papaState) === "loading"
    )
      return new Notice(t("notice.still_indexing"), 4000) && false;
    else if (get(papaState) === "error")
      return new Notice(t("notice.error"), 4000) && false;
    else if (get(papaState) !== "idle")
      return new Notice(t("notice.not_initialized"), 4000) && false;
    return true;
  }
  ...
  async createFilenameForChat() {
    let fileName = await this.papa.createTitleFromChatHistory(
      get(data).assistantLanguage,
      serializeChatHistory(get(chatHistory)),
    );
  ...
  getVectorStoreFile() {
    ...
    return normalizePath(
      this.pluginDir + "/vectorstores/" + selEmbedModel + ".bin",
    );
  }
  ...
  async saveVectorStoreData() {
    if (this.needsToSaveVectorStoreData) {
      ...
      (await this.app.vault.adapter.exists(this.pluginDir + "/vectorstores")) ||
        (await this.app.vault.adapter.mkdir(this.pluginDir + "/vectorstores"));
      await this.app.vault.adapter.writeBinary(
        this.getVectorStoreFile(),
        (await this.papa.getData()).buffer as ArrayBuffer,
      );
  ...
  async onFileChange(file: TFile) {
    if (!this.papa) return;
    for (const exclude of get(data).excludeFF)
      if (wildTest(exclude, file.path)) return;
    const docs = await obsidianDocumentLoader(
      await this.createPapaObsidianFiles([file]),
    );
