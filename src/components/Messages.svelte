<script lang="ts">
    ...
    const AIcolor = getThemeColor('--color-purple');
    ...
    {#each $messages as message (message.content)}
        {#if message.role === 'user'}
            <div class="flex justify-end mb-3">
                <p class="p-2 break-words rounded-t-md rounded-bl-md max-w-[80%] text-slate-0" style="background: {UserColor};" use:html={message.content} />
            </div>
        {:else}
            <div class="mb-3">
                <div id="test" class="p-2 break-words rounded-t-md rounded-br-md w-fit max-w-[80%]" style="background: {AIcolor};">
                    <p class="text-slate-0" use:html={message.content} />
