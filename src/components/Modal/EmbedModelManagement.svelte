...
<div class="modal-content">
	<div class="setting-item">
		<div class="setting-item-description">
			Here you can setup the embedding models that you want to use with s2b. The parameters that you specify for the models might not reflect their capabilities. If you are not sure what to do have a look at <a href="https://platform.openai.com/docs/models">OpenAI´s Model Page</a>
		</div>
	</div>

	<div class="grid p-3 gap-2 grid-cols-2 border-solid border-x-0 border-t border-b-0 border-[--background-modifier-border]">
		{#each embedModels as embedModel}
			<div class="community-item">
				<span>{embedModel[0]}</span>
				<span class="text-muted text-xs pt-1 leading-tight">{embedModel[1].similarityThreshold}</span>
			</div>
		{/each}
	</div>

	<SettingContainer name={'Embed Model Management'} desc={"Select and configure embedding models"}>
		{#if isPending}
			<div>is Loading</div>
		{:else}
			<Dropdown
				...
				dropdown={[
					{
						label: 'Configured',
						options: configuredModels.map(model => ({display: model, value: model}))
					},
					{
						label: 'Unconfigured',
						options: unconfiguredModels.map(model => ({display: model, value: model}))
					}
				]}
				onSelect={(model: string) => {
					selectedModel = model;
					if(isModelConfigured()) embedConfig = embedModels.get(selectedModel)!;
...
<div class="modal-button-container">
	<Button
		...
		onClick={() => selectedModel && data.addEmbedModel(provider, selectedModel, embedConfig)}
	/>
	<Button
		buttonText="Cancel"
		onClick={() => modal.close()}
