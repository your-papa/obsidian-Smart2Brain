...
const query = createQuery(() => ({
	...
	queryFn: async () =>
		await plugin.providerRegistry[selectedProvider].setup(data.getProviderAuthParams(selectedProvider)),
}));
...
<div class="modal-content">
    <div class="setting-item">
        <div class="setting-item-description">
            To use S2B with OpenAI, you need to add an API key. You can do this as follows.
            <li> Create a key by visiting <a href="https://platform.openai.com/api-keys">OpenAI`s Dashboard</a></li>
            <li> Ensure your OpenAI account is loaded up with credtis.</li>
            <li>Paste the API key below. It should start with '<strong>sk-</strong>'.</li>
        </div>
    </div>
		...
		{#each Object.entries(data.getProviderAuthParams(selectedProvider)) as [authKey, authValue]}
			<SettingContainer name={$t(`settings.provider.${authKey}`)} desc={$t(`settings.provider.${authKey}.desc`)}>
				<TextComponent
					inputType="text"
					value={authValue}
					styles={authValue === '' ? '' : query.data ? '!border-[--background-modifier-success]' : '!border-[--background-modifier-error]'}
...
<div class="modal-button-container">
   	{#if query.data !== undefined}
		<div class="flex items-center gap-2 rounded px-[--pill-padding-x] mr-auto"
			 class:bg-green-100={query.data}
			 class:bg-red-100={!query.data}>
			<div class="h-4 w-4"
				 class:text-green-600={query.data}
				 class:text-red-600={!query.data} use:icon={query.data ? 'check' : 'x'}></div>
			<span>
				{query.data ? 'Provider authentication successful' : 'Provider authentication failed'}
			</span>
		</div>
	{/if}
	<Button
