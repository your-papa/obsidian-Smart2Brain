...
// Generate preview URLs for image attachments
...
$effect(() => {
	...
	return () => {
		// Cleanup object URLs if we ever switch to URL.createObjectURL
	};
});
...
<div
	class="group relative {className} {isTruncated ? 'cursor-pointer' : ''}"
	onclick={handleClick}
>
			...
			<div class="flex flex-wrap gap-2 mb-2">
				{#each attachments as att (att.vaultPath)}
					{#if att.mimeType.startsWith("image/")}
						...
						{#if url}
							<img
								src={url}
								alt={att.name}
								class="max-h-24 max-w-32 rounded object-cover"
							/>
						{:else}
					...
					{:else if att.mimeType === "application/pdf"}
						<div class="flex items-center gap-1 text-xs text-[--text-muted] bg-[--background-modifier-hover] rounded px-2 py-1">
