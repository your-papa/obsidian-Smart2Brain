...
<div class="w-full">
  ...
  >
    ...
    <div class="flex flex-col gap-1.5 mt-2 pl-2 border-l-2 border-bg-modifier-border">
      {#each toolCalls as toolCall (toolCall.id)}
        {#if toolCall.preamble}
          <MarkdownRenderer
            content={toolCall.preamble}
            class="message-text markdown-preview-view leading-[1.5] !p-0 !w-full !max-w-full !m-0 [&_p]:my-2 [&_p:first-child]:mt-0 [&_p:last-child]:mb-0"
          />
        {/if}
        <div
        ...
        >
          <details>
            ...
            >
              <span class="text-[1.1em] min-w-[1.3em] text-center flex items-center justify-center">
              ...
              <span class="font-semibold text-[0.9rem] tracking-[0.01em] text-text-normal">
            ...
            >
              <div class="flex flex-col gap-2">
                ...
                {#if formatToolInput(toolCall.input).length > 0}
                  <div class="flex flex-col gap-2 mt-2">
                    {#each formatToolInput(toolCall.input) as { key, value } (key)}
                      ...
                      >
                        <span class="text-text-accent text-s">{key}:</span>
                        <MarkdownRenderer
                ...
                {:else}
                  <span class="text-text-muted italic text-[0.85rem] mt-2 block">(empty)</span>
                {/if}
              </div>
