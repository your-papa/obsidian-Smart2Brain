...
$effect(() => {
	console.log(pluginData.targetFolder);
});
...
<QueryClientProvider client={plugin.queryClient}>
    ...
    <SettingContainer name={'Chat Name'} desc={'Default Name for new chat'}>
        <Text inputType="text" placeholder="New Chat" value={pluginData.defaultChatName} blurFunc={(value: string) =>  pluginData.defaultChatName = value} />
    </SettingContainer>
    <SettingContainer name={'Chats Folder'} desc={'Where should your chats be saved to?'}>
        <FolderSuggest app={plugin.app} value={pluginData.targetFolder} suggestionFn={() => suggestFolders()} onSubmit={(value: string) => pluginData.targetFolder = value} onSelected={(value: string) => pluginData.targetFolder = value}/>
    </SettingContainer>
    ...
    <SettingContainer name={$t('settings.debugging')} isHeading={true} />
        <SettingContainer name={$t('settings.langsmith_key')} desc={$t('settings.langsmith_key_desc')}>
            <Text placeholder="ls__1c...4b" value={pluginData.debuggingLangchainKey} changeFunc={() => console.log('Changing Key')} />
        </SettingContainer>
        <SettingContainer name={$t('settings.verbose')} desc={$t('settings.verbose_desc')}>
            <ToggleComponent isToggled={pluginData.isVerbose} changeFunc={() => console.log('Change verbose')} />
        </SettingContainer>
    </details>
</QueryClientProvider>
