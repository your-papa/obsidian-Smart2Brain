<script lang="ts">
	...
	function changeTypeBadgeClass(entry: PendingChangeEntry): string {
		switch (entry.change.type) {
			case "create": return "ssb-badge-create";
			case "update": return "ssb-badge-update";
			case "delete": return "ssb-badge-delete";
		}
	}
	...
	function statusClass(entry: PendingChangeEntry): string {
		switch (entry.status) {
			case "pending": return "ssb-status-pending";
			case "accepted": return "ssb-status-accepted";
			case "rejected": return "ssb-status-rejected";
		}
	}
	...
	{:else}
		...
		{#if pendingCount > 0}
			<div class="ssb-pending-bulk">
				<span class="ssb-pending-count">{pendingCount} pending change{pendingCount !== 1 ? "s" : ""}</span>
