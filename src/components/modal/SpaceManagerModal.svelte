...
const hasAutoIncludeRules = $derived.by(() => parsedMembership.isAdvanced || autoIncludeRuleCount > 0);
const autoIncludeRulesOpen = $derived.by(() => parsedMembership.isAdvanced || showAutoIncludeRules);
...
function updateSimpleMembershipDraft(mutator: (draft: NonNullable<typeof parsedMembership.draft>) => void) {
