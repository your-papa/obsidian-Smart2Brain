...
import { fetchModelsDevData, lookupModelInfoSync, type ModelsDevModelInfo } from "../../providers/modelsDevApi";
...
// Fetch models.dev data on mount
onMount(async () => {
	modelsDevData = await fetchModelsDevData();
	// Focus search input
	searchInputEl?.focus();
});
...
// Get model metadata from models.dev
function getModelInfo(providerId: string, modelId: string): ModelsDevModelInfo | null {
	if (!modelsDevData) return null;
	return lookupModelInfoSync(modelsDevData, providerId, modelId);
}
...
// Handle clear selection (for embedding models)
...
<div class="model-selection-container">
	...
	<!-- Clear selection option for embedding models -->
	{#if modelType === "embedding"}
		<div
			role="button"
			tabindex="0"
			class="model-card clear-card"
			class:selected={!currentSelection}
			onclick={handleClear}
			onkeydown={(e) => e.key === 'Enter' && handleClear()}
		>
			<div class="model-main">
				<Icon name="x-circle" size="sm" class="text-[--text-muted]" />
				<div class="model-info">
					<div class="model-name">None (disable semantic search)</div>
					<div class="model-description">Lexical search only</div>
				</div>
			</div>
		</div>
	{/if}
						...
						>
							...
							<div class="model-meta">
								...
								<span class="meta-tag" title="Context window">
									{formatContextWindow(info?.limit?.context)} ctx
								</span>

								<!-- Cost -->
								{#if info?.cost?.input !== undefined || info?.cost?.output !== undefined}
									<span class="meta-tag" title="Cost per 1M tokens (in/out)">
										{formatCost(info?.cost?.input)}/{formatCost(info?.cost?.output)}
									</span>
								{/if}
