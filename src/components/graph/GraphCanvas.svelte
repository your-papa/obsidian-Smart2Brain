<script lang="ts">
  ...
  let nodeSize = $derived(
    Math.max(2, Math.round(7 - Math.log10(Math.max(graphData.nodes.length, 10)) * 1.8)),
  );
  ...
  let baseEdgeAlpha = $derived(
    Math.max(0.18, Math.min(0.6, 1.0 / Math.sqrt(Math.max(graphData.edges.length, 4)))),
  );
  ...
  let cachedDefaultLinkStrengthFn: ((link: SimLink, i: number, links: SimLink[]) => number) | null =
    null;
    ...
    if (showClusterAnchors) {
      ...
      for (const [cluster, node] of clusterRepresentativeNodes) {
        ...
        const pillY = Math.max(
          8,
          Math.min(height - ANCHOR_PILL_H - 8, screen.y - ANCHOR_PILL_H - 6),
        );
        ...
        for (let i = 0; i < anchorPlacements.length; i++) {
          ...
          for (let j = i + 1; j < anchorPlacements.length; j++) {
            ...
            if (overlapX && overlapY) {
              const overlapDepthY =
                Math.min(a.y + a.pillH + ANCHOR_GAP, b.y + b.pillH + ANCHOR_GAP) -
                Math.max(a.y, b.y);
              const overlapDepthX =
                Math.min(a.x + a.pillW + ANCHOR_GAP, b.x + b.pillW + ANCHOR_GAP) -
                Math.max(a.x, b.x);
    ...
    simNodes = data.nodes.map((n) => {
      ...
      if (old) {
        ...
        if (hasAnyKnown) {
          ...
          const angle =
            (2 * Math.PI * newNodeIndex) / Math.max(1, data.nodes.length - oldPositions.size);
  ...
  function setupForceSimulation(
    ...
    const defaultLinkStrengthFn = baseLinkForce.strength() as (
      link: SimLink,
      i: number,
      links: SimLink[],
    ) => number;
  ...
  $effect(() => {
    ...
    const clusterForce = sim.force("cluster") as
      | ReturnType<typeof clusterCohesionForce>
      | undefined;
