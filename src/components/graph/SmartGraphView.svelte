<script lang="ts">
      ...
      if (segmentBy === "semantic") {
        ...
        if (!documents) {
          showSettingsLinkNotice(
            plugin.app,
            "Semantic cluster coloring requires indexed embeddings.",
            {
              tab: "graph",
  ...
  async function ensureDocumentsLoaded(signal: AbortSignal): Promise<DocumentVector[] | null> {
    ...
    const configuredIndexCount = indexId
      ? (data.getEmbeddingIndex(indexId)?.documentCount ?? null)
      : null;
  ...
  async function computeAndApplyClusters(
    ...
    if (!filteredResult) {
      ...
      Logger.info(
        `[SmartGraphCache] Filtered cached (${formatCount(filteredResult.filteredDocs.length)} docs)`,
      );
    } else {
      Logger.info(
        `[SmartGraphCache] Filtered HIT (${formatCount(filteredResult.filteredDocs.length)} docs)`,
      );
    }
    ...
    const currentPathSetKey = filteredDocs
      ...
      .join("\0");
    const docSetChanged =
      clusterMap.size === 0 || cachedPathSetKey == null || currentPathSetKey !== cachedPathSetKey;
  ...
  $effect(() => {
    ...
    if (lastDocCacheKey !== null && key !== lastDocCacheKey) {
      Logger.info(
        `[SmartGraphCache] Index changed (${lastDocCacheKey} → ${key}), invalidating cache`,
      );
