 ...
 *
 * Each node is either:
 *   - a **leaf**: type dropdown (folder/tag/extension/paths) + value input
...
const leafTypeOptions = [
	...
	{ display: "Paths (frozen)", value: "paths" as LeafType },
];
...
function handleAddLeaf() {
	...
	onchange({ ...group, conditions: [...group.conditions, ...extra, { type: "folder", value: "" }] });
...
function handleAddGroup() {
	const group = filter as ViewFilterGroup;
	const extra = liveLeaf ? [liveLeaf] : [];
	onchange({ ...group, conditions: [...group.conditions, ...extra, { type: "all", conditions: [] }] });
	if (liveLeaf) onLiveLeafChange?.(null);
}
				...
				{#if liveLeaf}
					...
					<div class="filter-node filter-node--nested">
						<div class="filter-leaf-row">
							<Dropdown
								type="options"
								dropdown={leafTypeOptions}
								selected={liveType}
								onchange={(t) => onLiveLeafChange?.({ type: t, value: t === "paths" ? [] : "" } as ViewFilterLeaf)}
								class="filter-leaf-type"
							/>
							{#if liveLeaf.type === "paths"}
								<Text
									inputType="text"
									value={(liveLeaf.value as string[]).join(", ")}
									placeholder="path1, path2, …"
									onchange={(v: string) => onLiveLeafChange?.({ type: "paths", value: v.split(",").map((s) => s.trim()).filter(Boolean) })}
									class="filter-leaf-value"
								/>
							{:else if hasCombo(liveLeaf.type)}
											...
											{#each getSuggestions(liveType, comboQuery) as suggestion (suggestion)}
												<li>
													<button
														...
														onmousedown={(e) => { e.preventDefault(); pickSuggestion(suggestion, "live"); }}
													>{suggestion}</button>
												</li>
											{/each}
										</ul>
							...
							{:else}
								<Text
									...
									value={liveLeaf.value as string}
									placeholder="md"
									onchange={(v: string) => onLiveLeafChange?.({ ...liveLeaf!, value: v } as ViewFilterLeaf)}
									class="filter-leaf-value"
								/>
							{/if}
	...
	{:else}
		...
		<div class="filter-leaf-row">
			...
			{#if leafType === "paths"}
				<Text
					inputType="text"
					value={(filter as ViewFilterLeaf).value as unknown as string}
					placeholder="path1, path2, …"
					onchange={handleLeafValueChange}
					class="filter-leaf-value"
				/>
			{:else if hasCombo(leafType)}
							...
							{#each getSuggestions(leafType, comboQuery) as suggestion (suggestion)}
								<li>
									<button
										type="button"
										class="filter-combo-option"
										onmousedown={(e) => { e.preventDefault(); pickSuggestion(suggestion, "leaf"); }}
									>{suggestion}</button>
								</li>
							{/each}
						</ul>
