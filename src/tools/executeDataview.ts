...
export function createExecuteDataviewTool(app: App) {
    const executeDataviewFn = async ({ query }: { query: string }): Promise<string> => {
        // @ts-ignore - Dynamic access to plugins
        const dataviewApi = app.plugins.plugins["dataview"]?.api as DataviewAPI | undefined;
    ...
    return tool(executeDataviewFn, {
        ...
        description: "Execute an Obsidian Dataview query (DQL) and return the results in Markdown format. Use this to query notes, metadata, tags, and more using the Dataview Query Language. Example: 'LIST FROM \"Daily Notes\"'",
        schema: z.object({
            query: z
                .string()
